...
class NovelDownloaderMulti(NovelDownloader):
    ...
    def SaveToFileProcess(self, queue, startChap, numChapter):
        '''
        :param nd: NovelDownloaderMulti instance
        :param queue: multiprocessing.Manager().Queue()
        :param startChap: process(chapter) start index
        :param numChapter: process(chapter) max index
        :return:
        '''
        chapterWaitDict = {}
        idxNext = startChap
    ...
    def GetNovel(self, startChap = 1):
        ...
        # write process
        writeProc = mp.Process(target=self.SaveToFileProcess, args=(queue, startChap, numChapter))
